...
(function () {
	...
	gameFlow: function (){
		if (Snake.enter == true) {Snake.Pauseing(); Snake.enter = false;}
		if (Snake.gamestate != Snake.PAUSE){
			Snake.updateSnake();
	...
	updateSnake: function (){
		for (var snk=0; snk<Snake.players; snk++){
			...
			Snake.head = Snake.snake[Snake.snake.length-1];
			if (Snake.head[0] == Snake.apple[0] && Snake.head[1] == Snake.apple[1]){
				if (Snake.gamestate != Snake.DEMO) {Snake.updateEat(snk);}
			}
			Snake.updateComputers(snk);
			Snake.directions[snk] = Snake.savedirs[snk]
			
			if (Snake.head[3] == "dead" && Snake.aplate.length > 0 && Snake.aplate[0][2] == snk) {Snake.aplate.splice(0,1)}
			...
			if (Snake.head[3] != 'dead' ){
				...
				if (Snake.aplate.length > 0 && Snake.aplate[0][0] == Snake.snake[0][0] && Snake.aplate[0][1] == Snake.snake[0][1]) {Snake.aplate.splice(0,1);}
				else{
					if (Snake.head[3] != 'dead'){
